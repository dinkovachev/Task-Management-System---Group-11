...
//
//        return changeTaskSize(size, taskIndex);
//    }
//
//    private String changeTaskSize(Size size, int taskIndex) {
//        Task task = getTaskManagementSystemRepository().findTaskByID(taskIndex);
//        if (size.equals(task.getSize())) {
//            throw new IllegalArgumentException(format("Size is already set to %s", task.getSize()));
//        } else {
//            task.changeSize(size);
//            return format("Size changed to %s", task.getSize().toString());
//        }
//    }
//
